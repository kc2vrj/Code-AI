...
async function generateCode(prompt) {
  ...
  try {
    ...
    const textGenerationPipeline = await pipeline("text-generation", "Xenova/starcoderbase-1b");
...
async function main() {
  ...
  while (true) {
    ...
    try {
      ...
      logToFileSyncWithDetails("Generated Code", { length: generatedCode.length });
